      ...
      <div className="crud-layout">
        <form className="form-card" onSubmit={submitEmployee}>
          ...
          <div className="form-grid two-column">
            ...
            <label>Position<select value={draft.positionId ?? ''} onChange={(event) => setDraft({ ...draft, positionId: event.target.value })}><option value="">Unassigned</option>{positions.map((position) => <option key={position.id} value={position.id}>{position.title}</option>)}</select></label>
